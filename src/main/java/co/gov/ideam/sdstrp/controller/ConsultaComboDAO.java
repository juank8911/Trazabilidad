...
//			Expression<ResultType> expression = cb.coalesce(sbq, cb.literal(null) literal((ResultType) defaultResult);
	...
	public List<Residuos> listaResiduosGenTran(int idGes, int idTrans, int idSed) {
		CriteriaBuilder cb = em.getCriteriaBuilder();
		...
		try {
			CriteriaQuery<Residuos> conRes = cb.createQuery(Residuos.class);	
			 ...
			 conRes.where(cb.and(cb.equal(RootRes.get("res_sede_generador"), idSed),cb.equal(RootRes.get("res_sede_transporte"), idTrans),cb.equal(RootRes.get("res_sede_gestor"), idGes)));
			 conRes.select(RootRes);
